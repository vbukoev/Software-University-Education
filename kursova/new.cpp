...
#define N 3 // Тази стойност се променя спрямо нуждите на изпитването
...
int main() { // main функцията, в която се извършват основните действия и се извикват методите, които са инициализирани по-горе
    float A[N][N]; // А -> матрица с N*N
    float K, L; //инициализация на 2 реални числа, които се ползват за начална и крайна стойност на интервал

    printCondition();//условие на задачата
    printAuthor();// отпечва се името на автора

    inputArray(A);//входните данни
