    ...
    public class Deserializer
    {
        ...
        public static string ImportProjects(TeisterMaskContext context, string xmlString)
        {
            ...
            foreach (var projectDto in projectDtos)
            {
                ...
                bool isProjectDueDateValid = DateTime.TryParseExact(projectDto.DueDate, "dd/MM/yyyy",
                    CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime parsedProjectDueDate);

                if (!IsValid(projectDto) || !isProjectOpenDateValid || (!string.IsNullOrWhiteSpace(projectDto.DueDate) && !isProjectDueDateValid))
                {
                    sb.AppendLine(ErrorMessage);
                    continue;
                }

                DateTime? validProjectDueDate;
                if (string.IsNullOrWhiteSpace(projectDto.DueDate))
                    validProjectDueDate = null;
                else
                    validProjectDueDate = parsedProjectDueDate;
                ...
                foreach (var taskDto in projectDto.Tasks)
                {
                    ...
                    if (!IsValid(taskDto) || !isTaskOpenDateValid || !isTaskDueDateValid || validTaskOpenDate < validProjectOpenDate || (validProjectDueDate != null && validTaskDueDate > validProjectDueDate))
                    {
                        sb.AppendLine(ErrorMessage);
                        continue;
                    }
        ...
        public static string ImportEmployees(TeisterMaskContext context, string jsonString)
        {
            ...
            foreach (var employeeDto in employeeDtos)
            {
                ...
                int[] validTasks = context.Tasks.Select(t => t.Id).ToArray();
